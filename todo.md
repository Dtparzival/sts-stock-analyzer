...
**目標：**
將股票詳情頁的線圖改成 K 線圖（蠟燭圖），顯示開盤價、最高價、最低價和收盤價，提供更豐富的價格資訊。

**功能需求：**
- [x] 修改 StockChart 組件，使用 Recharts 的 ComposedChart + Bar
- [x] 顯示開盤價（open）、最高價（high）、最低價（low）、收盤價（close）
- [x] 保留時間範圍切換功能（1天、5天、1個月、3個月、1年、5年）
- [x] 保留自訂日期範圍功能
- [x] 優化 K 線圖的顏色配置（綠漲紅跌，美股習慣）
- [x] 測試 K 線圖數據格式（5/5 測試通過）

**技術方案：**
1. 使用 Recharts 的 `ComposedChart` + 自定義 `CandlestickShape`
2. 或使用第三方 K 線圖庫（如 lightweight-charts）
3. 確保數據格式包含 OHLC（開高低收）
4. 根據市場習慣設定顏色（台股：紅漲綠跌，美股：綠漲紅跌）
...
**問題描述：**
K 線圖只顯示上下影線，沒有顯示蠟燭實體部分，導致無法看到開盤價和收盤價之間的價格區間。

**修正任務：**
- [x] 檢查 CandlestickShape 組件的渲染邏輯
- [x] 修正 K 線實體的繪製方式（改用 CandlestickLayer）
- [x] 確保上漲和下跌的 K 線有不同的顏色和填充
- [x] 測試修正後的 K 線圖顯示效果（6/6 測試通過）

**預期效果：**
- K 線圖應該顯示完整的蠟燭形狀（實體 + 上下影線）
- 上漲 K 線顯示為綠色實心
- 下跌 K 線顯示為紅色實心
- Tooltip 正確顯示 OHLC 數據

## 新增需求：整合 TradingView Lightweight Charts

**目標：**
將現有的 K 線圖替換為 TradingView 官方的 Lightweight Charts，提供更專業、更穩定的技術分析圖表。

**功能需求：**
- [x] 安裝 lightweight-charts npm 套件（v5.0.9）
- [x] 創建 TradingViewChart 組件封裝
- [x] 實作 K 線圖（Candlestick Series）
- [x] 保留時間範圍切換功能
- [x] 保留自訂日期範圍功能
- [x] 添加成交量柱狀圖（Volume Series）
- [x] 支援響應式設計（自動調整圖表大小）
- [x] 測試數據轉換和格式化（6/6 測試通過）

**技術優勢：**
1. TradingView 官方維護，穩定可靠
2. 專業的金融圖表庫，支援多種技術指標
3. 高性能渲染，支援大量數據點
4. 豐富的自定義選項和主題配置
5. 支援移動平均線、布林帶等技術指標
