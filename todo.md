...
### 1. 擴充快速問題模板（使用頻率追蹤）
- [x] 設計 quickQuestionUsage 資料庫表結構（questionText, usageCount, lastUsedAt）
- [x] 實作後端 API 記錄快速問題使用次數（aiChat.trackQuestionUsage）
- [x] 實作後端 API 獲取熱門快速問題（aiChat.getPopularQuestions）
- [x] 修改前端 AI 聊天組件，當用戶點擊快速問題時記錄使用次數
- [x] 實作動態快速問題按鈕（根據使用頻率排序顯示前 4-6 個）
- [x] 添加預設快速問題列表（初始狀態）
- [x] 測試快速問題使用頻率追蹤功能
- [x] 測試動態快速問題按鈕顯示

### 2. 多股票對比分析功能
- [x] 設計多股票對比的 UI 介面（輸入多個股票代碼）
- [x] 實作後端 API 獲取多股票數據（compare.getMultipleStocks）
- [x] 實作後端 API 進行多股票對比分析（compare.analyzeComparison）
- [x] 整合 LLM 生成對比分析報告（基本面、技術面、風險評估）
- [x] 創建對比分析結果展示組件（表格、圖表）
- [x] 實作多股票價格走勢對比圖表
- [x] 實作多股票財務指標對比表格（顯示價格、漲跌幅、貨幣）
- [x] 添加 AI 對話支援多股票對比問題（例如「比較 TSLA 和 AAPL」）
- [x] 在首頁添加「多股票對比分析」功能卡片
- [x] 測試多股票對比分析功能
- [x] 測試 AI 對話中的多股票對比問題
