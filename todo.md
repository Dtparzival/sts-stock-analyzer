...
### 視覺設計優化
- [x] 將表格式設計改為卡片式設計
- [x] 為每個歷史記錄添加漸層圖標背景（使用 History 或 Clock 圖示）
- [x] 添加卡片 hover 動畫效果（border 和 shadow 過渡）
- [x] 優化投資建議標籤的視覺呈現（買入=綠色漸層、持有=黃色漸層、賣出=紅色漸層）
- [x] 使用漸層背景和圓角設計，確保與全站風格一致

### 資訊層級優化
- [x] 將分析摘要改為展開/收合設計（預設收合，顯示前 80 字）
- [x] 添加「展開詳情」/「收合」按鈕，使用 framer-motion 實現流暢動畫
- [x] 優化分析時間和當時股價的排版（清晰的層次結構）
- [x] 突出投資建議標籤（較大字體、漸層背景、陰影效果）

### 互動功能優化
- [x] 為每個卡片添加「查看完整分析」按鈕（展開該次分析的完整內容）
- [x] 使用 framer-motion 添加進入動畫（stagger children 效果）
- [x] 優化關閉按鈕的位置和樣式（右上角 X 按鈕 + 底部關閉按鈕）

### 響應式設計
- [x] 確保卡片在手機版正常顯示（單欄，上下排列）
- [x] 確保卡片在桌面版正常顯示（左右分欄）
- [x] 優化按鈕觸控區域（最小 44x44px）

### 對話框標題優化
- [x] 使用漸層圖標背景和漸層文字效果
- [x] 添加副標題顯示股票代號和歷史記錄數量
- [x] 標題區域與內容區域使用邊框分隔

### 測試和驗證
- [x] 測試歷史記錄顯示功能（至少 3 筆記錄）
- [x] 驗證卡片動畫效果流暢性
- [x] 驗證展開/收合功能正常運作
- [x] 驗證響應式設計在不同螢幕尺寸下的表現
- [x] 確認全站風格一致性
...
### 篩選和排序功能
- [x] 在對話框頂部添加篩選器區域（使用 Select 組件）
- [x] 實作按投資建議類型篩選（全部/買入/持有/賣出）
- [x] 實作排序選項（按時間降序/升序、按股價降序/升序）
- [x] 篩選器和排序器使用漸層背景和全站風格一致的設計
- [x] 添加重置篩選按鈕
- [x] 測試篩選和排序功能正常運作

### 分析對比功能
- [x] 為每個歷史記錄卡片添加選擇框（Checkbox）
- [x] 限制最多選擇 2 筆記錄進行對比
- [x] 在對話框底部添加「對比分析」按鈕（選擇 2 筆時啟用）
- [x] 創建分析對比對話框組件（CompareAnalysisDialog）
- [x] 對比對話框顯示兩筆記錄的並排比較：
  - [x] 分析時間對比
  - [x] 當時股價對比（視覺化柱狀圖）
  - [x] 投資建議對比（標籤顏色區分）
  - [x] 分析摘要並排顯示
  - [x] 股價變化趨勢（從第一次分析到第二次分析）
- [x] 使用 framer-motion 添加對比對話框的進入動畫
- [x] 確保對比功能遵循全站風格一致性
- [x] 測試對比功能正常運作

### 整合和測試
- [x] 整合篩選、排序和對比功能到歷史記錄對話框
- [x] 測試所有功能的互動流程
- [x] 驗證響應式設計在不同螢幕尺寸下的表現
- [x] 確認全站風格一致性
...
### 視覺設計優化
- [x] 優化對話框標題區域（添加漸層圖標背景和漸層文字效果）
- [x] 優化表單欄位樣式（添加圖標、清晰的標籤層次）
- [x] 添加市場選擇器（台股/美股快速切換按鈕，漸層背景設計）
- [x] 統一按鈕設計（漸層背景、hover 動畫效果）
- [x] 添加輸入驗證和友好的錯誤提示

### 功能增強
- [x] 整合智能股票搜尋（自動完成建議，支援台股中文名稱和美股搜尋）
- [x] 添加即時價格顯示（輸入股票代碼後自動顯示當前市價）
- [x] 添加成本計算預覽（即時顯示總成本 = 持股數量 × 購買價格）
- [x] 優化日期選擇器（使用更友好的日期選擇器組件）

### 整合和測試
- [x] 測試台股添加流程（輸入代碼、中文名稱搜尋）
- [x] 測試美股添加流程（輸入代碼、公司名稱搜尋）
- [x] 驗證即時價格顯示功能
- [x] 驗證成本計算預覽功能
- [x] 驗證響應式設計在不同螢幕尺寸下的表現
- [x] 確認全站風格一致性

## AI 投資顧問視窗優化（遵循全站紫藍漸層風格）

### 視覺設計優化
- [ ] 優化對話框標題區域（添加漸層圖標背景和漸層文字效果）
- [ ] 優化 AI 圖標設計（使用漸層背景圓形容器）
- [ ] 優化訊息氣泡樣式（AI 訊息和用戶訊息使用不同的視覺風格）
- [ ] 優化輸入框設計（添加圖標、漸層 focus 效果、更大的觸控區域）
- [ ] 優化發送按鈕（漸層背景、hover 動畫效果）
- [ ] 統一色彩系統（遵循全站紫藍漸層主題）

### 互動功能優化
- [ ] 添加訊息進入動畫（使用 framer-motion）
- [ ] 優化載入狀態動畫（AI 思考中的視覺反饋）
- [ ] 添加快速問題建議（常見問題快速按鈕）
- [ ] 優化滾動行為（新訊息自動滾動到底部）
- [ ] 添加清空對話功能（重新開始對話）
...
### 視覺設計優化
- [x] 標題欄使用紫藍漸層背景（from-blue-600 via-purple-600 to-indigo-600）
- [x] AI 圖標使用漸層背景的圓形容器
- [x] 訊息氣泡使用新的圓角設計（rounded-2xl）
- [x] 用戶訊息使用紫藍漸層背景
- [x] AI 訊息使用淺色漸層背景（from-gray-50 to-gray-100）
- [x] 輸入框區域使用漸層背景
- [x] 優化按鈕的 hover 和 active 狀態

### 互動功能優化
- [x] 添加訊息進入動畫效果（fade-in + slide-up）
- [x] 優化載入狀態顯示（AI 正在思考中...）
- [x] 添加清空對話功能
- [x] 優化快速問題建議按鈕樣式
- [x] 添加最小化/展開動畫效果
...
### 資訊層級優化
- [x] 標題區域清楚顯示「AI 投資顧問」
- [x] 訊息區域使用不同顏色區分用戶和 AI
- [x] 輸入框區域有明確的視覺分隔
- [x] 按鈕功能一目了然（清空、最小化、關閉）

## 新增需求：AI 對話功能優化

### 1. 快速問題模板
- [ ] 在 AI 對話界面添加常見投資問題的快捷按鈕
- [ ] 設計問題模板列表（例如：分析我的投資組合、推薦低風險股票、市場趨勢分析等）
- [ ] 實作點擊按鈕自動填充問題到輸入框
- [ ] 優化 UI 佈局（模板按鈕放置位置）
- [ ] 測試快速問題模板功能

### 2. 智能股票數據整合
- [ ] 實作股票代碼檢測邏輯（從用戶消息中提取股票代碼）
- [ ] 創建後端 API 自動獲取股票即時數據
- [ ] 將股票數據整合到 AI 對話上下文
- [ ] 修改 AI prompt，讓 AI 能夠使用股票數據提供精準分析
- [ ] 在對話界面顯示股票數據卡片（可選）
- [ ] 測試股票數據整合功能（美股、台股）
- [ ] 驗證 AI 分析準確性
...
### 1. 快速問題模板
- [x] 在 AI 對話界面中添加常見投資問題的快捷按鈕
- [x] 實作模板按鈕點擊自動填充問題
- [x] 設計 6 個常用投資問題模板（分析投資組合、推薦低風險股票、市場趨勢分析等）

### 2. 智能股票數據整合
- [x] 檢測用戶消息中的股票代碼（支援美股和台股）
- [x] 獲取股票即時數據（使用 TwelveData API 和 TWSE API）
- [x] 將股票數據整合到 AI 回答中
- [x] 支援緩存機制，減少 API 請求
- [x] 實作單元測試（10/10 測試通過）
- [x] 瀏覽器測試驗證功能正常運作
...
### 問題 1：快速問題模板按鈕未顯示
- [x] 檢查 AIChatBox 組件中快速問題模板的實作
- [x] 修正 AIChatBox 組件的顯示邏輯（移除 displayMessages.length > 0 條件）
- [x] 修正 FloatingAIChat 組件缺少 quickTemplates 屬性
- [x] 確保按鈕在對話開始時正確顯示

### 問題 2：驗證股票數據整合功能
- [x] 測試股票代碼檢測功能（支援美股和台股）
- [x] 驗證單元測試通過（10/10 通過）
- [x] 確認 AI 能夠自動獲取股票數據並提供分析

## 新增需求：AI 投資顧問功能後續優化

### 1. 擴充快速問題模板
- [x] 設計快速問題使用頻率追蹤資料庫表結構（questionStats）
- [x] 實作後端 API 記錄快速問題使用次數
- [x] 實作後端 API 獲取最常使用的快速問題
- [x] 實作 AI 智能動態調整快速問題按鈕邏輯
- [x] 修改前端 AI 聊天組件，整合動態快速問題
- [x] 確保全站風格一致性（按鈕樣式、顏色、間距）
- [x] 測試快速問題頻率追蹤功能
- [x] 測試 AI 動態調整快速問題按鈕

### 2. 多股票對比分析
- [x] 設計多股票對比分析的數據結構
- [x] 實作股票代碼解析功能（識別「比較 TSLA 和 AAPL」等查詢）
- [x] 實作後端 API 獲取多支股票數據
- [x] 整合 LLM 進行多股票對比分析
- [x] 設計對比分析結果顯示格式（表格、圖表等）
- [x] 修改前端 AI 聊天組件，支援多股票對比分析
- [x] 添加多股票對比分析的快速問題模板
- [x] 確保全站風格一致性（表格樣式、圖表配色）
- [x] 測試多股票對比分析功能（2-5 支股票）
- [x] 測試跨市場對比分析（美股 vs 台股）

## 登出流程與響應式設計優化

- [x] 優化登出功能，確保登出後返回首頁未登入畫面
- [x] 強化首頁未登入狀態的視覺設計與內容展示
- [x] 優化全站響應式設計，確保手機版與平板版顯示效果
- [x] 確保跨裝置體驗一致性（桌面、平板、手機）
- [x] 測試登出流程與響應式佈局

## 優化登入後首頁的「為您推薦」區塊（2025-11-23）

### 視覺美觀性提升
- [x] 優化卡片樣式（陰影、圓角、hover 效果、漸層背景）
- [x] 添加圖標和視覺層次（股票圖標、市場標籤）
- [x] 優化標題和副標題設計
- [x] 添加進場動畫效果（使用 CSS 動畫和 transition）
- [x] 確保與全站風格一致（配色、字體、間距）

### 操作流暢性優化
- [x] 添加載入骨架屏（skeleton loading）
- [x] 優化卡片點擊反饋和互動效果（active:scale-95）
- [x] 添加快速操作按鈕（查看詳情、加入收藏）
- [x] 實作卡片 hover 時顯示更多資訊（快速操作按鈕）
- [x] 優化空狀態和錯誤狀態顯示

### 資訊層級清晰度
- [x] 重新設計資訊架構（股票代碼、名稱、最後查看時間）
- [x] 添加股票圖標或市場標籤（美股/台股）
- [x] 優化文字大小和顏色對比
- [x] 添加最後查看時間
- [x] 實作分類標籤（最近查看）
...
- [x] 優化平板版卡片佈局（3 欄）
- [x] 優化桌面版卡片佈局（4-6 欄）
- [x] 確保所有按鈕符合最小觸控區域（44x44px）
- [x] 測試不同螢幕尺寸的顯示效果

### 測試與驗證
- [x] 測試已登入用戶的推薦顯示
- [x] 測試未登入用戶的推薦顯示（不顯示推薦區塊）
- [x] 測試響應式設計在不同裝置上的表現
- [x] 測試動畫效果的流暢性
- [x] 驗證全站風格一致性
- [x] 儲存檢查點

## 修正「為您推薦」區塊的顯示數量（2025-11-23）

### 任務目標
調整登入後首頁的「為您推薦」區塊，確保在手機版、平板版、桌面版都只顯示 6 個推薦股票，保持響應式設計的一致性。

### 待辦事項

- [x] 檢查當前「為您推薦」區塊的實作邏輯
- [x] 找出控制顯示數量的代碼位置
- [x] 修改後端 API 或前端邏輯，限制推薦數量為 6 個
- [x] 優化響應式佈局（手機版 2 欄、平板版 3 欄、桌面版 3 欄）
- [x] 測試手機版顯示效果（確認只顯示 6 個）
- [x] 測試平板版顯示效果（確認只顯示 6 個）
- [x] 測試桌面版顯示效果（確認只顯示 6 個）
- [x] 驗證跨裝置體驗一致性

## 優化「為您推薦」區塊響應式佈局（桌面版改為 6 欄）

### 任務目標
調整登入後首頁的「為您推薦」區塊，優化響應式佈局為：
- 手機版（< 640px）：2 欄
- 平板版（≥ 640px, < 1024px）：3 欄
- 桌面版（≥ 1024px）：6 欄

確保在所有裝置都只顯示 6 個推薦股票，充分利用桌面版的寬螢幕空間。

### 待辦事項

- [x] 修改 Home.tsx 的推薦區塊網格佈局類別（grid-cols-2 sm:grid-cols-3 lg:grid-cols-6）
- [x] 同步更新載入骨架屏的網格佈局
- [x] 測試手機版顯示效果（2 欄，共 6 個）
- [x] 測試平板版顯示效果（3 欄，共 6 個）
- [x] 測試桌面版顯示效果（6 欄，共 6 個）
- [x] 驗證卡片寬度和間距在不同螢幕尺寸下的視覺效果
- [x] 確保全站風格一致性
- [x] 儲存檢查點
