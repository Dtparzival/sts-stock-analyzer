...
- [x] 將成交量從底部移至左側獨立縱軸
- [x] 調整 TradingView 圖表配置，使用左側 Y 軸顯示成交量
- [x] 確保 K 線圖和成交量不會重疊
- [x] 測試修正後的圖表顯示效果


## 新增優化：K 線圖 Tooltip 視窗進一步縮小和排版優化

- [x] 分析當前 Tooltip 的尺寸和排版(已縮小過一次,需要進一步優化)
- [x] 進一步縮小 Tooltip 視窗尺寸(最小寬度從 160px 縮至 130px,內邊距從 p-2 縮至 p-1.5)
- [x] 優化數據排版呈現方式(字體從 12px/10px 縮至 10px/9px,標籤簡化為單字母)
- [x] 改進視覺層次和資訊密度(間距從 gap-3/space-y-0.5 縮至 gap-2/space-y-0)
- [x] 測試優化後的 Tooltip 顯示效果(移動端和桌面端)
