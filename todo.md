...
**問題描述：**
K 線圖只顯示一條細線，沒有顯示完整的蜡燭圖實體和影線。Tooltip 顯示正確的 OHLC 數據，說明數據傳遞正確，但視覺渲染有問題。

**待修正項目：**
- [x] 檢查 CandlestickShape 組件的 SVG 繪製邏輯
- [x] 檢查 Y 坐標計算是否正確
- [x] 檢查 K 線寬度計算
- [x] 修正視覺渲染問題
- [x] 測試不同時間範圍的圖表顯示
...
**根本原因：**
Recharts 的 Bar 組件不會自動傳遞 `xAxisMap` 和 `yAxisMap` 到自定義 shape。使用 `shape` prop 無法獲取必要的坐標軸信息。

**修正方案：**
1. 將 `CandlestickShape` 改為 `CandlestickLayer`
