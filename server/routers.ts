  ...
  analysis: router({
    ...
    createImprovementPlan: protectedProcedure
      .input(z.object({
        ...
        problemTarget: z.string().optional(),
        problemDescription: z.string(),
        baselineAccuracy: z.number(),
        targetAccuracy: z.number(),
        improvementMeasures: z.string(),
        startDate: z.date().optional(),
        endDate: z.date().optional(),
        notes: z.string().optional(),
      }))
      .mutation(async ({ ctx, input }) => {
        const planId = await db.createImprovementPlan({
          userId: ctx.user.id,
          problemType: input.problemType,
          problemTarget: input.problemTarget || null,
          problemDescription: input.problemDescription,
          baselineAccuracy: Math.round(input.baselineAccuracy * 10000),
          targetAccuracy: Math.round(input.targetAccuracy * 10000),
    ...
    // 獲取改進計畫列表
    getImprovementPlans: protectedProcedure
      .query(async ({ ctx }) => {
        const plans = await db.getImprovementPlans(ctx.user.id);
        return plans.map(plan => ({
          ...plan,
          baselineAccuracy: plan.baselineAccuracy / 10000,
          targetAccuracy: plan.targetAccuracy / 10000,
          currentAccuracy: plan.currentAccuracy ? plan.currentAccuracy / 10000 : null,
          improvementRate: plan.improvementRate ? plan.improvementRate / 10000 : null,
        }));
      }),
    ...
    getImprovementPlan: protectedProcedure
      ...
      .query(async ({ input }) => {
        const plan = await db.getImprovementPlanById(input.id);
        if (!plan) {
          throw new TRPCError({
            code: 'NOT_FOUND',
            message: '改進計畫不存在',
    ...
    // 更新改進計畫
    updateImprovementPlan: protectedProcedure
      .input(z.object({
        id: z.number(),
        status: z.enum(["pending", "in_progress", "completed", "cancelled"]).optional(),
        currentAccuracy: z.number().optional(),
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.currentAccuracy !== undefined) updateData.currentAccuracy = Math.round(input.currentAccuracy * 10000);
        if (input.improvementRate !== undefined) updateData.improvementRate = Math.round(input.improvementRate * 10000);
        if (input.notes !== undefined) updateData.notes = input.notes;
        if (input.startDate !== undefined) updateData.startDate = input.startDate;
        if (input.endDate !== undefined) updateData.endDate = input.endDate;
        
        await db.updateImprovementPlan(input.id, updateData);
        return { success: true };
      }),
    
    // 刪除改進計畫
    deleteImprovementPlan: protectedProcedure
      .input(z.object({
        id: z.number(),
      }))
      .mutation(async ({ input }) => {
        await db.deleteImprovementPlan(input.id);
        return { success: true };
      }),
    
    // 獲取活躍的改進計畫
    getActiveImprovementPlans: protectedProcedure
      .query(async ({ ctx }) => {
        const plans = await db.getActiveImprovementPlans(ctx.user.id);
        return plans.map(plan => ({
          ...plan,
          baselineAccuracy: plan.baselineAccuracy / 10000,
          targetAccuracy: plan.targetAccuracy / 10000,
