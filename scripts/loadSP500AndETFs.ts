 ...
 * 
 * 使用方式:
 * pnpm tsx scripts/loadSP500AndETFs.ts
 * 
 * 參數:
...
} from '../server/jobs/syncUsStockData';
...
interface BatchResult {
  totalRecords: number;
  totalErrors: number;
  allErrors: Array<{ symbol?: string; message: string }>;
}
 ...
 * 載入股票清單
 */
function loadStockList(jsonFile: string | null): string[] {
  let filePath: string;
  ...
  const stocks = JSON.parse(content) as Array<{ symbol: string }>;
 ...
 * 批次處理股票基本資料
 */
async function processStockInfoInBatches(
  symbols: string[],
  batchSize: number,
  delaySeconds: number,
  startBatch: number
 ...
 * 批次處理股票歷史價格
 */
async function processStockPricesInBatches(
  symbols: string[],
  batchSize: number,
  delaySeconds: number,
  startDate: Date,
...
async function main() {
  ...
  // 步驟 1: 同步股票基本資料
  let infoResult: BatchResult | null = null;
  ...
  // 步驟 2: 同步歷史價格資料
  let priceResult: BatchResult | null = null;
  if (!params.infoOnly) {
    console.log('\n' + '='.repeat(70));
