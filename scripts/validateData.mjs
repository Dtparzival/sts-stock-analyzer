...
async function main() {
  try {
    ...
    const totalStocks = await db.select({ count: sql<number>`count(*)` }).from(twStocks);
    const activeStocks = await db.select({ count: sql<number>`count(*)` }).from(twStocks).where(eq(twStocks.isActive, true));
    const twseStocks = await db.select({ count: sql<number>`count(*)` }).from(twStocks).where(eq(twStocks.market, '上市'));
    const tpexStocks = await db.select({ count: sql<number>`count(*)` }).from(twStocks).where(eq(twStocks.market, '上櫃'));
    ...
    const totalPrices = await db.select({ count: sql<number>`count(*)` }).from(twStockPrices);
    ...
    const totalIndicators = await db.select({ count: sql<number>`count(*)` }).from(twStockIndicators);
    ...
    const totalFundamentals = await db.select({ count: sql<number>`count(*)` }).from(twStockFundamentals);
    ...
    const totalFinancials = await db.select({ count: sql<number>`count(*)` }).from(twStockFinancials);
    ...
    const totalDividends = await db.select({ count: sql<number>`count(*)` }).from(twStockDividends);
